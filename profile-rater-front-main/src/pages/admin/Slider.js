...
const Slider = () => {
    ...
    const Slider_get_quantity = Cookies.get('admin_get_quantity') ?? Cookies.set('admin_get_quantity', 10) ? Cookies.get('admin_get_quantity') : 10;
        
    const Slider_get_page = Cookies.get('admin_get_spage') ?? Cookies.set('admin_get_spage', 1) ? Cookies.get('admin_get_spage') : 1;
   

    const [res_data, setData] = useState(null);
    const [basePath, setBasePath] = useState('');
    const [get_quantity, setGetQuantity] = useState(Slider_get_quantity);
    const [rankingPage, setRankingPage] = useState(Slider_get_page);
    ...
    const fetchSliderData = async (get_quantity = 10, rankingPage = 1) => {
        try {
            const response = await SliderApi.get(get_quantity, rankingPage);
    ...
    const handlePage = (page) => {
        setRankingPage(page);
        Cookies.set('admin_get_spage', page);
    }

    useEffect(() => {
        fetchSliderData(get_quantity, rankingPage);
    ...
    const handleDelete = async (SliderId) => {
        try {
            await SliderApi.destroy(SliderId);
            setShowDeleteModal(false, 0);
            fetchSliderData(); // Refresh the Slider list
        ...
        <>
            <div className='w-full max-w-screen overflow-hidden lg:px-28 min-h-screen bg-color-6 dark:bg-color-4 space-y-5'>
                <div className='w-fit mx-auto pt-10'>
                    <button onClick={handleOpenModal} className="bg-color-1 hover:bg-color-1/80 w-fit px-10 rounded-md text-white py-3">Add Slide</button>
                </div>
                        ...
                        <table className="table table-auto max-lg:w-max w-full overflow-x-auto">
                            <thead>
                                <tr className=" border-b-[0.5px] border-gray-100 dark:border-color-4">
                                    ...
                                    <td className="text-xs text-color-4 dark:text-color-5 py-3 px-2">TITLE1</td>
                                    <td className="text-xs text-color-4 dark:text-color-5  py-3 px-2">TITLE2</td>
                                    
                                    <td className="text-xs text-color-4 dark:text-color-5 py-3 px-2">TITLE3</td>
                                    <td className="text-xs text-color-4 dark:text-color-5 py-3 px-2 text-center">BG COLOR</td>
                                    <td className="text-xs text-color-4 dark:text-color-5 py-3 px-2 text-center">SLIDE No.</td>
                                    <td className="text-xs text-color-4 dark:text-color-5 py-3 px-2 text-center">IN_SLIDE</td>                                 
                                        ...
                                        return <tr key={index} className=" border-b-[0.5px] border-gray-100 dark:border-color-4 *:py-3 *:px-2 group hover:bg-color-6 hover:dark:bg-color-4">
                                            ...
                                            <td className=" text-color-3 dark:text-color-5 max-lg:text-sm font-outfit-medium">
                                                <span className="flex items-center gap-6">
                                                    <div className="size-10 rounded-full overflow-hidden bg-color-5">
                                                        <img src={`${basePath}/${Slider.image}`} alt="" className="w-full h-full" />
                                                    </div>                                                    
                                                </span>
                                            </td>
                                            <td className=" text-color-3 dark:text-color-5 font-outfit-medium">{Slider.title1}</td>
                                            <td className=" text-color-3 dark:text-color-5 font-outfit-medium">{Slider.title2}</td>
                                           
                                            <td className=" text-color-3 dark:text-color-5 font-outfit-medium">{Slider.title3}</td>
                                            <td className="">                                                                                                   
                                                    <div className={`size-5 rounded-sm  mx-auto border-[0.5px] border-gray-300`} style={{backgroundColor:`${Slider.bg_color}`}}></div>                                                                                                   
                                            </td>
                                            <td className=" text-color-3 dark:text-color-5 font-outfit-medium text-center">{Slider.slide_number}</td>
                                            <td className="">
                                                {
                                            ...
                                            <td className=' w-fit sticky right-0 bg-white dark:bg-color-2 group-hover:bg-color-6 group-hover:dark:bg-color-4'>
                                                <div className='invisible group-hover:visible flex gap-2 bg-inherit w-20'>
                                                    ...
                                                    <button onClick={() => handleDeleteModal(true, Slider.id)}>
                                    ...
                                    <>
                                        {
                                            ...
                                            <tr>
                                                <td colSpan={6}>
                                                    <div className='dark:text-color-6'>No Slider Found</div>
                                                </td>
                                            </tr>
                                        }
                        ...
                        >
                            <div className="">
                                ...
                                <div className="p-2 mt-2 text-center space-x-1 md:block">
                                    ...
                                    <button
                                        onClick={() => handleDelete(selectedSlider)}
