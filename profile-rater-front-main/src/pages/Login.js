...
import {login,getSocialLoginUrl} from '../api/user/auth/userAuth';
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    login(username, password).then((response) => {     
        ...
        if (location.state?.from !== '/signup' && location.state?.from !== '/forgot-password' && location.state?.from !== '/reset-password') {
          navigate(location.state?.from || -1,{state:{from:'login'}});
        } else {
          navigate('/'); // Redirect to home or another page if the previous was signup
        }
  ...
  const handleSocialLogin = async (provider) => {
    try {
      const response = await getSocialLoginUrl(provider);      
        ...
        <section className='min-h-screen'>
          <div className="h-full w-full max-lg:px-2 lg:my-16 my-5">
            <div className="bg-color-9 dark:bg-color-2 min-h-[90vh] w-full lg:w-2/5 mx-auto relative lg:px-16 px-5 py-8 text-center space-y-10">
              <div className="font-outfit-bold text-5xl text-color-3 dark:text-color-6">Login</div>
              <div className="w-full">
                ...
                {message && <p className="mt-4 text-color-4 dark:text-color-5">{message}</p>}
                    ...
                    <Link to="/signup">
                      <div className="text-center text-color-4 dark:text-color-5 cursor-pointer underline mt-5">SignUp</div>
                    
                    </Link>
                    <Link to="/admin">
                      <div className="text-center text-color-4 dark:text-color-5 cursor-pointer underline mt-5">Admin Login</div>
